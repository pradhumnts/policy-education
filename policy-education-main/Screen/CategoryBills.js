...
import BillList from "../PageComponent/BillList"
...
const CategoryBills = () => {
    ...
    const [filteredBill, setFilteredBill] = useState(BillData);

    const handleSearch = (text) => {
        setSearchValue(text);
    setFilteredBill(BillData.filter((item) => item.summary.toLowerCase().includes(text.toLowerCase())));
    }
  ...
  return (
   <View>
        ...
        <FlatList
        data={filteredBill}
        renderItem={({ item }) => <BillList item={item} />}
